...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.alter_column('rb_socialauth', 'auth_token', self.gf('django.db.models.fields.CharField')(unique=True, max_length=103))

        # Adding unique constraint on 'SocialAuth', fields ['auth_token']
        db.create_unique('rb_socialauth', ['auth_token'])

        # Changing field 'SocialAuth.expires'
        db.alter_column('rb_socialauth', 'expires', self.gf('django.db.models.fields.DateTimeField')(null=True))

        # Adding unique constraint on 'SocialAuth', fields ['auth_token', 'expires']
        db.create_unique('rb_socialauth', ['auth_token', 'expires'])

        # Adding unique constraint on 'SocialUser', fields ['uid']
        db.create_unique('rb_socialuser', ['uid'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'SocialUser', fields ['uid']
        db.delete_unique('rb_socialuser', ['uid'])

        # Removing unique constraint on 'SocialAuth', fields ['auth_token', 'expires']
        db.delete_unique('rb_socialauth', ['auth_token', 'expires'])

        # Removing unique constraint on 'SocialAuth', fields ['auth_token']
        db.delete_unique('rb_socialauth', ['auth_token'])

        # Changing field 'SocialAuth.auth_token'
        db.alter_column('rb_socialauth', 'auth_token', self.gf('django.db.models.fields.CharField')(max_length=255))

        # User chose to not deal with backwards NULL issues for 'SocialAuth.expires'
        raise RuntimeError("Cannot reverse this migration. 'SocialAuth.expires' and its values cannot be restored.")
