...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'Interaction.updated'
        db.delete_column('rb_interaction', 'updated')

        # Deleting field 'Interaction.inserted'
        db.delete_column('rb_interaction', 'inserted')

        # Adding field 'Interaction.created'
        db.add_column('rb_interaction', 'created', self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, default=datetime.date(2011, 3, 1), blank=True), keep_default=False)

        # Adding field 'Interaction.modified'
        db.add_column('rb_interaction', 'modified', self.gf('django.db.models.fields.DateTimeField')(auto_now=True, default=datetime.date(2011, 3, 1), blank=True), keep_default=False)

        # Deleting field 'Content.updated'
        db.delete_column('rb_content', 'updated')

        # Deleting field 'Content.inserted'
        db.delete_column('rb_content', 'inserted')

        # Adding field 'Content.created'
        db.add_column('rb_content', 'created', self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, default=datetime.date(2011, 3, 1), blank=True), keep_default=False)

        # Adding field 'Content.modified'
        db.add_column('rb_content', 'modified', self.gf('django.db.models.fields.DateTimeField')(auto_now=True, default=datetime.date(2011, 3, 1), blank=True), keep_default=False)


    def backwards(self, orm):
        
        # User chose to not deal with backwards NULL issues for 'Interaction.updated'
        raise RuntimeError("Cannot reverse this migration. 'Interaction.updated' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'Interaction.inserted'
        raise RuntimeError("Cannot reverse this migration. 'Interaction.inserted' and its values cannot be restored.")

        # Deleting field 'Interaction.created'
        db.delete_column('rb_interaction', 'created')

        # Deleting field 'Interaction.modified'
        db.delete_column('rb_interaction', 'modified')

        # User chose to not deal with backwards NULL issues for 'Content.updated'
        raise RuntimeError("Cannot reverse this migration. 'Content.updated' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'Content.inserted'
        raise RuntimeError("Cannot reverse this migration. 'Content.inserted' and its values cannot be restored.")

        # Deleting field 'Content.created'
        db.delete_column('rb_content', 'created')

        # Deleting field 'Content.modified'
        db.delete_column('rb_content', 'modified')
