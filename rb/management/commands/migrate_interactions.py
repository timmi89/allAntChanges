...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        op_paginator = Paginator(old_pages, 10)
        for x in op_paginator.page_range:
            for p in op_paginator.page(x).object_list:
                logger.info('migrating: ' + p.canonical_url)
                try:
                    new_url = p.canonical_url.replace('http://' + old_group.short_name, '')
                    new_page = Page.objects.filter(canonical_url = 'http://www.' + new_group.short_name + new_url, site__group = new_group)
