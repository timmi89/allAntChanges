...
function FatFractal() {
     ...
     */
    var m_version = "FF_JS_CS_SDK_R1.2.0_R2656";
    ...
    */
    ...
    this.postObjToExtension = function (obj, extensionUri, successCallback, errorCallback) {
        if(! successCallback) successCallback = m_ff.defaultSuccessCallback;
        if(! successCallback) throw new Error("FatFractal.postObjToExtension: successCallback not supplied");
        if(! errorCallback) errorCallback = m_ff.defaultErrorCallback;
...
//                successCallback(retVal, response.statusMessage);
    ...
    */
    ...
    /**
    <br>Retrieve FFDL on the app's backend, if the user has permission to do so.
    @param {Function} successCallback Will be invoked with one argument - the response from the backend
    @param {Function} errorCallback Will be invoked with one argument - the xmlHTTP object
     */
    ...
    var m_loadAllReferences = function(referringObject, done) {
        ...
        if (m_autoLoadRefs) {
            if (referringObject.ffRefs != null && referringObject.ffRefs.length != 0) {
                ...
                for (var refItemIx = 0; loadedRefs < refsToLoad && refItemIx < referringObject.ffRefs.length; refItemIx++) {
    ...
    var m_loadReference = function(refItem, referringObj, done) {
        if (m_debug) console.log("m_loadReference: referringObj = " + JSON.stringify(referringObj));
        ...
        if (cachedRef == null) {
            if (refItem.type == "FFO" || (refItem.type == "FFB" && m_autoLoadBlobs)) {
