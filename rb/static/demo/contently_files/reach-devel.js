...
SPR.provide('DOM', {
  ...
  /**
   * Adds an iframe tag to the head of the document in a synchronous fashion
   *
   */
   addIframeSync: function(src) {
     var frame = document.createElement('iframe'),
         container = document.body;
     frame.setAttribute('name', 'spr');
     frame.src = src;
     ...
     if(!container){
       container = SPR.DOM.$('BODY')[0];
     }

     if(!container){
       container = SPR.DOM.$('HEAD')[0];
     }
...
SPR.provide('Reach', {
  ...
  collect: function(params){
    ...
    options.referrer = params.referrer || document.referrer;
    options.page_url = params.page_url || window.location.toString();
    ...
    if (options.pid === null || options.pid === undefined || !options.url || !options.date){
    ...
    if (params.iframe){
      options.ref_url = options.ref_url || document.referrer;
      options.cache_buster = new Date().getTime();
      var qs = SPR.QS.encode(options);
      SPR.DOM.addIframeSync('//' + SPR.Reach.cdn + '/iframe.html?' + qs);

    } else {
      if(options.pid === 0){
        options.pid = '000000000000000000000000';
      }
