		...
		searchbox.onkeyup = function(e) {
			if (sSearchId == "searchfield"){
				...
				if (e && (e.keyCode === 40 || e.keyCode == 38)) {
					...
					var resultsJSON = "http://search." + thisEdition + ".reuters.com/query/suggestion.do?q=" + encodeURIComponent(searchbox.value) + "&site=" + thisEdition + "&resultsPerPage=5&callback=Reuters.utils.storeSuggestedSearchNewsResults";
					Reuters.utils.loadScript("suggestedSearchJSON", resultsJSON);
...
Reuters.utils.storeSuggestedSearchNewsResults = function(o) {
  ...
  var edition = (o.edition == 'US') ? 'www' : o.edition.toLowerCase();
	...
	var resultsJSON = "http://" + edition + ".reuters.com" + searchUrl + "/jsonCompanySearch?searchType=symbol&search=" + encodeURIComponent(searchStr) + "&callback=Reuters.utils.storeSuggestedSearchSymbolResults";
	Reuters.utils.loadScript("suggestedSearchSymbolJSON", resultsJSON);
}
...
Reuters.utils.storeSuggestedSearchSymbolResults = function(o) {
  Reuters.utils.suggestedSearchResults.quoteSearchResult = o;
	...
	if (Reuters.utils.suggestedSearchResults.quoteSearchResult.length <= 0) {
		...
		var resultsJSON = "http://"+thisEdition+".reuters.com" + searchUrl + "/jsonCompanySearch?searchType=name&search=" + encodeURIComponent(searchStr) + "&callback=Reuters.utils.storeSuggestedSearchNameResults";
		Reuters.utils.loadScript("suggestedSearchNameJSON", resultsJSON);
	} else {
...
Reuters.utils.loadSuggestedSearch = function(o){
	...
	function populateResults() {
		...
		var host = "http://" + thisEdition + ".reuters.com";
...
Reuters.utils.loadScript = function(scriptName, scriptUrl) {
...
Reuters.utils.loadStylesheet = function(cssName, cssUrl) {
...
Reuters.utils.replaceContent = function(elementId, url, interval, reloadCallbackFunc){
	var replacer = new Reuters.utils._contentReplacer(elementId, url, interval, reloadCallbackFunc);
	replacer.update();
}

Reuters.utils._contentReplacer = function(elementId, url, interval, reloadCallbackFunc){
  ...
  if (location.href.search("grid=true") != -1) {
    ...
    for (i=0; i<allDivs.length; i++) {
      if (allDivs[i].className.search("section") != -1) {
        allDivs[i].style.cssText += ' background: url("http://design.reuters.com/prototypes/redesign/bg_grid.gif") top left';
      }
    }
  }
}
...
Reuters.utils.login = function(arg) {
	if ((Reuters.info === null) || (typeof Reuters.info === 'undefined')) {
		var thisEdition = "us";
	} else {
		var thisEdition = (Reuters.info.edition.search(/betaus/i) == -1 ? Reuters.info.edition.toLowerCase() : "us");
	...
	if (Reuters.utils.isLoggedIn() == false) {
		...
		if (thisEdition == 'us') {
			...
			if(arg == 'COMMENTS'){ 
				overlayBox.innerHTML = '<div id="modalLoginFlow"><div class="closer" onclick="Reuters.utils.closeLogin()"></div><iframe id="loginFrame" src="https://commerce.'+thisEdition+'.reuters.com/login/pages/login/loginEmbedded.do?go=' + encodeURIComponent(location.href) + '&success=http://' + location.hostname + '/assets/loginSuccessful&flow=COMMENTS" width="490" height="550" marginwidth="0" marginheight="0" scrolling="no" frameborder="0"></iframe></div>';
			} else {
...
Reuters.utils.loadHeaderLinks = function() {
	if ((Reuters.info === null) || (typeof Reuters.info === 'undefined')) {
		var thisEdition = "us";
	} else {
		if ((Reuters.info.edition === null) || (typeof Reuters.info.edition === 'undefined')) {
	...
	if (document.getElementById("utilities")) {
		var utilityHTML = '<ul>';
		if (Reuters.utils.isLoggedIn() == false) {
			utilityHTML += '<li class="last"><a href="https://commerce.'+thisEdition+'.reuters.com/registration/pages/registration/begin.do?go=' + encodeURIComponent(location.href) + '">Register</a></li>';
			if (thisEdition == 'us') {
				...
				utilityHTML += '<li class="last"><a href="https://commerce.'+thisEdition+'.reuters.com/login/pages/login/portfolioLogin.do?go=' + encodeURIComponent(location.href) + '">Sign In</a></li>';
			}
		} else {
			utilityHTML += '<li><a href="https://commerce.'+thisEdition+'.reuters.com/profile">My Account</a></li>';
			if (YAHOO.util.Cookie.get("socialLoginProvider")){
				var socProvider = YAHOO.util.Cookie.get("socialLoginProvider");
				trace ("socialLoginProvider: " + socProvider);
				utilityHTML += '<li id="loggedin-social" onmouseover="Reuters.utils.showSocialLoggedInFlyout()" onmouseout="Reuters.utils.hideSocialLoggedInSoon();"><div class="socProvider ' + socProvider + '"> </div></li>'; 
			}
			if (Reuters.info.edition != 'IN') {
				utilityHTML += '<li><a href="https://commerce.' + thisEdition + '.reuters.com/purchase/mycart.do">My Cart</a></li>';
			}
			utilityHTML += '<li><a href="https://commerce.'+thisEdition+'.reuters.com/login/pages/login/portfolioLogin.do?go=http://portfolio.'+thisEdition+'.reuters.com/'+thisEdition.toUpperCase()+'/overview.asp&go_withoutlogin=http://portfolio.'+thisEdition+'.reuters.com/'+thisEdition.toUpperCase()+'/public/index.asp">Portfolio</a></li>';
			utilityHTML += '<li class="last"><a href="javascript:Reuters.utils.logout();">Logout</a></li>';
		}
		if (thisEdition == 'us') {
			if (typeof document.forms["gigyaForm"] == "undefined") {
				utilityHTML += '<div id="gigya-controller"><form action="https://commerce.'+thisEdition+'.reuters.com/login/pages/login/login.do?go=' + encodeURIComponent(location.href) + '" method="post" name="gigyaForm"><input type="hidden"	value="827" name="__MOID__" /><input type="hidden"	value="CheckGigyaResponseForm" name="__FH__" /><input type="hidden" name="uid"/><input type="hidden" name="signature"/><input type="hidden" name="timestamp"/><input type="hidden" name="email"/><input type="hidden" name="backUrl"/><input type="hidden" name="backParameterEncoded"/><input type="hidden" name="source"/><input type="hidden" name="loginProvider"/><input type="hidden" name="providers"/></form><div id="gigyalogin"></div></div>';
			} else {
...
Reuters.utils.addLoadEvent(Reuters.utils.loadHeaderLinks);
		...
		if (document.getElementById("signin-social").offsetHeight != 0) {
			if (document.getElementById("login-social-flyout")) {
				...
				if (typeof gigya == 'undefined'){
					Reuters.utils.loadScript("gigyaservices", "https://cdns.gigya.com/JS/gigya.js?services=socialize");
				}
				var optionalHost = '';
				...
				Reuters.utils.loadScript("gigyacreds", optionalHost + "/resources_v2/js/rcom-login-gigya.js");
				var h = '<div id="loadScreen"><img src="' + optionalHost + '/resources_v2/images/icon_social_loading.gif" border="0" alt="Loading..." /></div>';
				d.innerHTML = h;
...
Reuters.utils.showLoginOptionsViaGigya = function() {
	if (document.getElementById("login-social-flyout")) {
		...
		var h = '';
		h = '<div class="biglabel">Sign in with <a href="https://commerce.'+thisEdition+'.reuters.com/login/pages/login/portfolioLogin.do?go=' + encodeURIComponent(location.href) + '">Reuters</a>...</div>'
...
Reuters.utils.showSocialLoggedInFlyout = function() {
	if (document.getElementById("header") || document.getElementById("reutersHeader")) {
		if (document.getElementById("social-loggedin-flyout")) {
			document.getElementById("social-loggedin-flyout").className = "";
			Reuters.utils.dontHideSocialLoggedIn();
		} else {
			var socProvider = YAHOO.util.Cookie.get("socialLoginProvider");
			var allSocProviders = ["facebook","google","yahoo","linkedin","aol","twitter","myspace"];
			var d = document.createElement("div");
			d.id = "social-loggedin-flyout";
			if (typeof gigya == 'undefined'){
				Reuters.utils.loadScript("gigyaservices", "https://cdns.gigya.com/JS/gigya.js?services=socialize");
			}
			var optionalHost = '';
			...
			Reuters.utils.loadScript("gigyacreds", optionalHost + "/resources_v2/js/rcom-login-gigya.js");
			var h = '<div class="label">You\'re logged in using: </div>' 
			h += '<div class="gigyaLoggedIn ' + socProvider + '"><span class="providerName">' + socProvider + '</span></div>';
			h += '<div class="manageAcct"><div class="biglabel"><a href="https://commerce.us.reuters.com/profile">Manage My Account...</a></div></div>';
			h += '<div class="socialOptions"><div class="biglabel">Link Other Accounts...</div><ul>';
			for (i=0; i<allSocProviders.length; i++){
				if (allSocProviders[i] != socProvider){
					h += '<li title="' + allSocProviders[i] + '" class="' + allSocProviders[i] + '" onclick="gigya.services.socialize.plugins.login.providerClick(\'gigyalogin\',\''+ allSocProviders[i] + '\')"></li>';
				}
			}
			h += '</ul></div>';	
			d.innerHTML = h;
			d.onmouseover = Reuters.utils.dontHideSocialLoggedIn;
...
Reuters.utils.hideSocialLoggedInSoon = function() {
	Reuters.utils.hideSocialLoggedinTimer = setTimeout(Reuters.utils.hideSocialLoggedIn, 100);
}
Reuters.utils.dontHideSocialLoggedIn = function() {
	clearTimeout(Reuters.utils.hideSocialLoggedinTimer);
}
Reuters.utils.hideSocialLoggedIn = function() {
	if (document.getElementById("social-loggedin-flyout")) {
		document.getElementById("social-loggedin-flyout").className = "hidden";
	}
}
...
Reuters.utils.closeLogin = function() {
	...
	if (Reuters.utils.isLoggedIn() == true) {
		Reuters.tns.reloadLoginStatus();
		Reuters.utils.loadHeaderLinks();
...
Reuters.utils.showOverlayCurtain = function(width, height, top) {
	...
	var checkWindow = Reuters.utils.getInnerXY();

	if (checkWindow.height<=height){
			var documentHeight = document.getElementById("header").offsetHeight + document.getElementById("content").offsetHeight + (height - checkWindow.height) + top + 10;
		}
	if (checkWindow.height>height){
			var documentHeight = document.getElementById("header").offsetHeight + document.getElementById("content").offsetHeight + top + 10;
		}
	if (checkWindow.width<=980){
			checkWindow.width = 980;
			documentHeight = documentHeight + 17;
		}

	if (checkWindow.width>980){
		if (is_ie5up){
			checkWindow.width = (checkWindow.width + 17);
			}
		checkWindow.width = (checkWindow.width - 17);
		}
			top = Reuters.utils.getScrollXY().y + top;
			document.getElementById("overlayMask").style.cssText = "height: " + documentHeight + "px; width: " + checkWindow.width + "px;";
	...
	//If IE6+
		if (!document.getElementById("iframeHackForIE")) {
			var selectElems = document.getElementsByTagName("select");
			if(selectElems.length >0){
				for(x=0;x<selectElems.length;x++){
					selectElems[x].style.zIndex="-1";
				}
			}

			document.getElementById("overlayMask").style.zIndex="0";
			var iframeHackForIE = document.createElement("iframe");
			iframeHackForIE.id = "iframeHackForIE";
			iframeHackForIE.setAttribute("scrolling", "no");
			iframeHackForIE.setAttribute("class", "iframeHackForIE");
			iframeHackForIE.setAttribute("width", checkWindow.width);
			iframeHackForIE.setAttribute("height", documentHeight);
			iframeHackForIE.setAttribute("src", "javascript:'<html></html>';");
			document.body.insertBefore(iframeHackForIE, document.getElementById("overlayMask"));
		}
		else {
			document.getElementById("iframeHackForIE").style.display = 'block';
		}
	}
...
Reuters.utils.hideOverlayCurtain = function() {
	if (document.getElementById("overlayMask")) {
		...
		//IF IE6+
		if (document.documentElement && (document.documentElement.clientWidth || document.documentElement.clientHeight) && document.getElementById("iframeHackForIE")) {
		document.getElementById("iframeHackForIE").style.display = 'none';		
		}
...
Reuters.utils.submitSearchVideo = function() {
	var searchTextVideo = document.getElementById('videoSearchField');
	var searchTextValueVideo = searchTextVideo.value;
	if (searchTextValueVideo  == '' || searchTextValueVideo == 'Search Videos') {
		return false;
	}
	else {
		return true;
...
if (Reuters.utils.isLoggedIn() == true){
        ...
        try{
                ...
                Reuters.utils.loadScript("commerceSurveyModal", commerceSurveyCheckUrl);
...
if (typeof(YAHOO) != "undefined") {
        var commerceSurveyCheckEventAdded = YAHOO.util.Cookie.get("commerceSurveyCheckEventAdded");
        if (commerceSurveyCheckEventAdded != null ){

        }else{
                Reuters.utils.addLoadEvent(Reuters.utils.doCommerceSurveyCheck);
                YAHOO.util.Cookie.set("commerceSurveyCheckEventAdded", 'true');
        }
}


Reuters.utils.setInfoCookie = function() {
	var ed = 'US'
	if (typeof Reuters.info != "undefined") {
		if (Reuters.info.edition != 'BETAUS') {
			ed = Reuters.info.edition;
		}
	}
	YAHOO.util.Cookie.setSub("info", "edition", ed, { domain: "reuters.com" } );
}

Reuters.utils.loadScript("errorPage", "/resources_v2/js/rcom-wt-mlt.js");
