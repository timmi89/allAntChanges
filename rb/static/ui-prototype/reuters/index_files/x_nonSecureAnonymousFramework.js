...
var l,
    ...
    valid = false,
    a = "",
    fwk = "http://platform.linkedin.com/js/framework?v=0.0.1120-RC2.6304-1337",
    xtnreg = /extensions=([^&]*)&?/,
...
*/
  ...
  l.xd_html = "https://platform.linkedin.com/js/xdrpchtml?v=0.0.1120-RC2.6304-1337";
  l.xd_us_html = "http://platform.linkedin.com/js/xdrpchtml?v=0.0.1120-RC2.6304-1337";
  l.api_xd_html = "https://api.linkedin.com/uas/js/xdrpchtml?v=0.0.1120-RC2.6304-1337";
  ...
  l.userspace_renew = "https://www.linkedin.com/uas/js/authuserspace?v=0.0.1120-RC2.6304-1337&api_key={API_KEY}";
  l.base_js_url = "http://static01.linkedin.com/scds/concat/common/js?v=build-1120_2_6304-prod&f=js%2Fconnect%2FPATH_INFO";
  ...
  l.sprite = "http://static02.linkedin.com/scds/common/u/img/sprite/sprite_connect_v11.png";

  /* todo praveen - high. These are backwards! */
  l.unsecure_xdswf = "http://static.linkedin.com/scds/common/u/js/connect/dev/lib/easyxdm.swf";
  l.secure_xdswf = "https://www.linkedin.com/scds/common/u/js/connect/dev/lib/easyxdm.swf";
...
}function getLocation(url){var m=url.match(reURI);
...
}function addSwf(domain){easyXDM.stack.FlashTransport[domain]={queue:[]};
...
}else{if(!easyXDM.stack.FlashTransport[swfdomain]){addSwf(swfdomain)
...
}).Method("processTagMatch",function(f){var d=f.tagName.toLowerCase(),b,c=false,m=false;
switch(d){case"in:tag":b=f.getAttribute("name").toLowerCase();
m=true;
if(/\+init$/.test(b)){return false
}break;
case"script":var p=f.type.toLowerCase();
c=true;
if(/\+init$/.test(p)){return false
}var g=p.split("/");
var n=g[0];
if(n!="in"){return false
}b=g[1];
break
}if(!this.tagRegistry[b]){return false
}if(m){f.setAttribute("name",b+"+init")
}if(c){f.type=f.type+"+init"
}var j,a,e=f.innerHTML,l={};
for(var h=0,q=f.attributes,k=q.length;
h<k;
h++){if(IN.Util.support.li_attributesSpecified){var o=q.item(h).nodeName;
o=o.replace("data-","").toLowerCase();
l[o]=q.item(h).nodeValue
}else{if(q.item(h).specified){var o=q.item(h).nodeName;
o=o.replace("data-","").toLowerCase();
l[o]=q.item(h).nodeValue
}}}l.content=e;
...
a.className="IN-widget";
f.parentNode.insertBefore(a,f);
j=new this.tagRegistry[b](a,l);
return true
...
(function(){var c;
Sslac.Class("IN.Objects.Base").Constructor(function(h){var g=document.createElement("span"),i={"padding":"0","margin":"0","text-indent":"0","display":"inline-block","vertical-align":"middle"};
if(!h.suppressFont){i["font-size"]="1px"
}g.style.cssText=this.createStyle(i);
this.el=function(){return g
};
this.components={};
this.componentId=null;
this.placed=false;
this.handlers=[]
}).Method("removeHandler",function(h,g){return function(p,r,m,k){if(this.isPlaced()){IN.Event.remove(h,g,p,r,m,k)
}else{var l=this.handlers;
var j=[];
for(var n=0,o=l.length;
n<o;
n++){var q=l[n];
if(q.id!==h||q.on!==g||q.fn!==p||q.scope!==r||q.obj!==m||q.once!==k){j.push(q)
}}this.handlers=j
}}
}).Method("addHandler",function(i,g){var h=function(l,k,m,j){var l=l;
var n=function(o,p){if(!h.enabled){return false
}if(p){l.call(this,o,p)
}else{l.call(this,o)
}};
if(this.isPlaced()){IN.Event.on(i,g,n,k,m,j)
}else{this.handlers[this.handlers.length]={id:i,on:g,fn:n,scope:k,obj:m,once:j}
}};
h.enabled=true;
return h
}).Method("createTemplate",function(g){return IN.Util.createJSTemplate(g)
}).Method("place",function(){var j=[].slice.apply(arguments),h="",n=null,l=this.el();
if(!j[0]){h="in";
n=document.body
}else{if(!j[1]){h="in";
n=j[0]
...
n=j[1]
}}switch(h){case"in":n.appendChild(l);
break;
case"before":n.parentNode.insertBefore(l,n);
break;
case"after":if(n.nextSibling){n.nextSibling.parentNode.insertBefore(l,n.nextSibling)
}else{n.parentNode.appendChild(l)
}break
}this.placed=true;
for(var k=0,g=this.handlers.length;
k<g;
k++){var m=this.handlers[k];
IN.Event.on(m.id,m.on,m.fn,m.scope,m.obj,m.once)
}this.handlers=[];
return this
}).Method("remove",function(){var g=this.el();
if(g.parentNode){g.parentNode.removeChild(g)
}this.placed=false
}).Method("isPlaced",function(){return this.placed
}).Method("createStyle",function(j){var h,g=[],i="";
if(!c){h=navigator.userAgent;
c=(h.match(/Webkit\//i))?"Webkit":(h.match(/Gecko\//i))?"Gecko":(h.match(/MSIE/))?"MSIE":"other"
}for(name in j){if(typeof(j[name])=="object"){i=j[name][c]||j[name]["other"]
}else{i=j[name]
}if(!i){continue
}g[g.length]=name+":"+i+" !important;"
}return g.join("")
}).Method("setComponentId",function f(g){this.componentId=g
}).Method("registerComponent",function d(h,g,i){i=i||"normal";
this.components[h]=g
}).Method("getComponent",function a(h,j,i){var g=this.components[h]||"",j=j||"",i=i||"";
g=g.replace(/\{ID\}/g,this.componentId).replace(/\{\.STATE\}/g,(j)?"."+j:"").replace(/\{\#STATE\}/g,(j)?"#"+j:"").replace(/\{:PSEUDO\}/g,(i)?":"+i:"");
return g
}).Method("createCSS",function b(l,k,m){if(!IN.Util.isArray(l)){l=[l]
}var j=[],m=(typeof m==="undefined")?"":m+" ";
for(var h=0,g=l.length;
h<g;
h++){j.push(m+this.getComponent(l[h].component,l[h].state,l[h].pseudo))
}return[j.join(", "),"{",k,"}"].join("")
}).Method("addCSS",function e(g){return IN.Util.addCSS(g)
}).Method("generateId",function(g){g=(g)?"li_ui_"+g:"li_ui";
return IN.$uid(g)
})
})();
...
this.sizeSettings={small:{height:20,width:20,logo:[0,-220],logoHover:[-20,-220],logoClick:[-40,-220],logoPadding:4,font:11,titleHeight:18,titleLine:20,ieOffset:2,borderRadius:2},large:{height:26,width:26,logo:[-90,-140],logoHover:[-120,-140],logoClick:[-150,-140],logoPadding:4,font:13,titleHeight:23,titleLine:23,ieOffset:2,borderRadius:3},xlarge:{height:48,width:48,logo:[0,-170],logoHover:[-50,-170],logoClick:[-100,-170],logoPadding:4,font:24,titleHeight:46,titleLine:46,ieOffset:2,borderRadius:5}};
var b="",f=this.generateId(),e={},d=false,a=this;
b=["",'<span id="'+f+'">','<a id="'+f+'-link" href="javascript:void(0);">',(c.useLogo)?'<span id="'+f+'-logo">in</span>':"",(c.title)?'<span id="'+f+'-title"><span id="'+f+'-mark"></span><span id="'+f+'-title-text">'+c.title+"</span></span>":"","</a>","</span>",""].join("");
...
}).Method("getBaseStyles",function(a){return["",this.createCSS({component:"link"},this.createStyle({"height":"1%"}),"* html"),this.createCSS({component:"outer"},this.createStyle({"position":"relative","overflow":"visible","display":"block"})),this.createCSS({component:"link"},this.createStyle({"border":"0","height":this.getSetting("height")+"px","text-decoration":"none","padding":"0","margin":"0","zoom":{MSIE:"1"},"display":{MSIE:"inline",other:"inline-block"}})),this.createCSS([{component:"link",pseudo:"link"},{component:"link",pseudo:"visited"},{component:"link",pseudo:"hover"},{component:"link",pseudo:"active"}],this.createStyle({"border":"0","text-decoration":"none"})),this.createCSS({component:"link",pseudo:"after"},this.createStyle({"content":'"."',"display":"block","clear":"both","visibility":"hidden","line-height":"0","height":"0"})),""].join("")
}).Method("getLogoStyles",function(a){if(!a.useLogo){return""
}return["",this.createCSS({component:"logo"},this.createStyle({"background":"url("+IN.ENV.images.sprite+") "+this.getSetting("logo").join("px ")+"px no-repeat","cursor":"pointer","border":"0","text-indent":"-9999em","overflow":"hidden","padding":"0","margin":"0","z-index":"1","position":(a.title)?"absolute":"","left":(a.title)?"0px":"","top":(a.title)?"0px":"","display":"block","width":this.getSetting("width")+"px","height":this.getSetting("height")+"px","float":"left"})),this.createCSS({component:"logo",state:"hovered"},this.createStyle({"background-position":this.getSetting("logoHover").join("px ")+"px"})),this.createCSS([{component:"logo",state:"clicked"},{component:"logo",state:"down"}],this.createStyle({"background-position":this.getSetting("logoClick").join("px ")+"px"})),""].join("")
}).Method("getTitleStyles",function(a){if(a.title===""){return""
}return["",this.createCSS({component:"title"},this.createStyle({"font-size":this.getSetting("font")+"px","font-family":"Arial, sans-serif","font-weight":"bold","color":"#333","cursor":"pointer","display":"block","float":"left","margin-left":"1px","vertical-align":"top","height":{MSIE:(this.getSetting("titleHeight")-this.getSetting("ieOffset"))+"px",other:this.getSetting("titleHeight")+"px"},"padding":{MSIE:"0 "+this.getSetting("logoPadding")+"px "+this.getSetting("ieOffset")+"px "+(this.getSetting("width")+this.getSetting("logoPadding")-1)+"px",other:"0 "+this.getSetting("logoPadding")+"px 0 "+(this.getSetting("width")+this.getSetting("logoPadding")-1)+"px"},"border":"1px solid #000","border-top-color":"#E2E2E2","border-right-color":"#BFBFBF","border-bottom-color":"#B9B9B9","text-shadow":"#FFFFFF -1px 1px 0","border-left":"0","line-height":{MSIE:(this.getSetting("height")-(this.getSetting("ieOffset")*2))+"px",other:this.getSetting("height")+"px"},"border-radius":(a.useLogo)?"0":this.getSetting("borderRadius")+"px","-webkit-border-radius":{Webkit:(a.useLogo)?"0":this.getSetting("borderRadius")+"px"},"-moz-border-radius":{Gecko:(a.useLogo)?"0":this.getSetting("borderRadius")+"px"},"border-top-right-radius":this.getSetting("borderRadius")+"px","border-bottom-right-radius":this.getSetting("borderRadius")+"px","-webkit-border-top-right-radius":{Webkit:this.getSetting("borderRadius")+"px"},"-webkit-border-bottom-right-radius":{Webkit:this.getSetting("borderRadius")+"px"},"-moz-border-radius-topright":{Gecko:this.getSetting("borderRadius")+"px"},"-moz-border-radius-bottomright":{Gecko:this.getSetting("borderRadius")+"px"},"background-color":{Webkit:"",Gecko:"",MSIE:"",other:"#ECECEC"},"background-image":{Webkit:"-webkit-gradient(linear, left top, left bottom, from(#FEFEFE), to(#ECECEC))",Gecko:"-moz-linear-gradient(top,  #FEFEFE,  #ECECEC)"},"filter":{MSIE:"progid:DXImageTransform.Microsoft.gradient(startColorstr='#FEFEFE', endColorstr='#ECECEC', GradientType=0)"}})),this.createCSS({component:"title",state:"hovered"},this.createStyle({"color":"#000","border":"1px solid #000","border-top-color":"#ABABAB","border-right-color":"#9A9A9A","border-bottom-color":"#787878","border-left-color":(a.useLogo)?"#04568B":"#ABABAB","border-left":(a.useLogo)?"0":"1px","background-color":{Webkit:"",Gecko:"",MSIE:"",other:"#DEDEDE"},"background-image":{Webkit:"-webkit-gradient(linear, left top, left bottom, from(#EDEDED), to(#DEDEDE))",Gecko:"-moz-linear-gradient(top,  #EDEDED,  #DEDEDE)"},"filter":{MSIE:"progid:DXImageTransform.Microsoft.gradient(startColorstr='#EDEDED', endColorstr='#DEDEDE', GradientType=0)"}})),this.createCSS([{component:"title",state:"clicked"},{component:"title",state:"down"}],this.createStyle({"color":"#666","border":"1px solid #000","border-top-color":"#B6B6B6","border-right-color":"#B3B3B3","border-bottom-color":"#9D9D9D","border-left-color":(a.useLogo)?"#49627B":"#B6B6B6","border-left":(a.useLogo)?"0":"1px","background-color":{Webkit:"",Gecko:"",MSIE:"",other:"#DEDEDE"},"background-image":{Webkit:"-webkit-gradient(linear, left top, left bottom, from(#E3E3E3), to(#EDEDED))",Gecko:"-moz-linear-gradient(top,  #E3E3E3,  #EDEDED)"},"filter":{MSIE:"progid:DXImageTransform.Microsoft.gradient(startColorstr='#E3E3E3', endColorstr='#EDEDED', GradientType=0)"}})),this.createCSS({component:"titleText"},this.createStyle({"display":"inline-block","vertical-align":"top","height":{MSIE:(this.getSetting("titleHeight")-this.getSetting("ieOffset"))+"px",other:this.getSetting("titleHeight")+"px"},"line-height":{MSIE:(this.getSetting("titleLine")-this.getSetting("ieOffset")*2)+"px",other:this.getSetting("titleLine")+"px"}})),this.createCSS({component:"mark"},this.createStyle({"display":"inline-block","width":"0px","overflow":"hidden"})),""].join("")
});
...
b=["",'<span id="'+h+'-container" class="'+c.position+(c.state=="hidden"?" hidden":"")+'">','<span id="'+h+'" class="'+c.position+'">','<span id="'+h+'-inner" class="'+c.position+'">','<span id="'+h+'-content" class="'+c.position+'">'+c.content+"</span>","</span>","</span>","</span>",""].join("");
g=["","#{ID}-container.right {",this.createStyle({"display":"inline-block","float":"left","overflow":"visible","position":"relative","height":"18px","padding-left":"2px","line-height":"1px","cursor":"pointer","vertical-align":{MSIE:"-2px"}}),"}","#{ID}.right {",this.createStyle({"display":"block","float":"left","height":"18px","margin-right":"4px","padding-right":"4px","background":"url("+IN.ENV.images.sprite+") right -100px no-repeat"}),"}","#{ID}-inner.right {",this.createStyle({"display":"block","float":"left","padding-left":"8px","text-align":"center","background":"url("+IN.ENV.images.sprite+") 0px -60px no-repeat"}),"}","#{ID}-content.right {",this.createStyle({"display":"inline","font-size":"11px","color":"#04558B","font-weight":"bold","font-family":"Arial, sans-serif","line-height":"18px","padding":"0 5px 0 5px"}),"}","#{ID}-container.hovered #{ID}.right, #{ID}-container.clicked #{ID}.right, #{ID}-container.down #{ID}.right {",this.createStyle({"background-position-y":"-120px"}),"}","#{ID}-container.hovered #{ID}-inner.right, #{ID}-container.clicked #{ID}-inner.right, #{ID}-container.down #{ID}-inner.right {",this.createStyle({"background-position-y":"-80px"}),"}","#{ID}-container.empty {",this.createStyle({"display":"none"}),"}",""].join("").replace(/\{ID\}/g,h);
e=["","#{ID}-container.top {",this.createStyle({"display":"inline-block","overflow":"visible","position":"relative","height":"42px","line-height":"1px","cursor":"pointer"}),"}","#{ID}.top {",this.createStyle({"display":"inline-block","height":"42px","width":"57px","background":"url("+IN.ENV.images.sprite+") -150px top no-repeat"}),"}","#{ID}-content.top {",this.createStyle({"display":"inline","font-size":"16px","color":"#04558B","font-weight":"bold","font-family":"Arial, sans-serif","line-height":"34px"}),"}","#{ID}-container.hovered #{ID}.top, #{ID}-container.clicked #{ID}.top, #{ID}-container.down #{ID}.top {",this.createStyle({"background-position-x":"-210px"}),"}","#{ID}-container.empty #{ID}-inner.top {",this.createStyle({"background":"url("+IN.ENV.images.sprite+") 0px -20px no-repeat","overflow":"hidden","margin":"5px auto 0 auto","width":"26px","height":"26px","display":"block"}),"}","#{ID}-container.empty #{ID}-content.top {",this.createStyle({"text-indent":"-999px","display":"inline-block"}),"}",""].join("").replace(/\{ID\}/g,h);
f=["",(c.position=="right")?g:"",(c.position=="top")?e:"","#{ID}-container.hidden #{ID} {",this.createStyle({"display":"none"}),"}",""].join("").replace(/\{ID\}/g,h);
IN.Util.addCSS(f);
this.el().innerHTML=b;
...
IN.Util.removeClass(b,"empty");
if(c=="0"||c==""){IN.Util.addClass(b,"empty")
}a.innerHTML=c
...
switch(c){case"hidden":IN.Util.addClass(a,"hidden");
break;
case"visible":IN.Util.removeClass(a,"hidden");
break;
case"mouseOver":IN.Util.addClass(a,"hovered");
break;
case"mouseDown":IN.Util.addClass(a,"down");
break;
case"mouseOut":IN.Util.removeClass(a,"hovered");
...
this.postUrl=a.url;
this.transportUrl=(this.postUrl.match(/^https:\/\//))?IN.ENV.url.xd_html:IN.ENV.url.xd_us_html;
this.xdSwfUrl=(this.postUrl.match(/^https:\/\//))?IN.ENV.images.secure_xdswf:IN.ENV.images.unsecure_xdswf;
this.postParams={};
...
this.postParams=a;
return this
}).Method("ready",function ready(b,a){this.callbacks.ready.push({fn:b||function(){},scope:a||window});
return this
}).Method("success",function success(b,a){this.callbacks.success.push({fn:b||function(){},scope:a||window});
return this
}).Method("error",function error(b,a){this.callbacks.error.push({fn:b||function(){},scope:a||window});
return this
}).Method("handleWidgetReady",function handleWidgetReady(e,f,c){if(this.callbacks.ready.length){for(var d=0,b=this.callbacks.ready.length;
d<b;
d++){var a=this.callbacks.ready[d];
a.fn.call(a.scope,e)
}}f()
}).Method("handleWidgetSuccess",function handleWidgetSuccess(e,f,c){if(this.callbacks.success.length){for(var d=0,b=this.callbacks.success.length;
d<b;
d++){var a=this.callbacks.success[d];
a.fn.call(a.scope,e)
}}f()
}).Method("handleWidgetFailure",function handleWidgetFailure(e,f,c){if(this.callbacks.error.length){for(var d=0,b=this.callbacks.error.length;
d<b;
d++){var a=this.callbacks.error[d];
a.fn.call(a.scope,e)
}}f()
...
break;
case"modal":g={embedded:false,overlay:true};
break;
...
}function i(){IN.Event.remove(IN,"refresh",c.triggerRefresh,c);
IN.Event.remove(IN,"logout",c.triggerRefresh,c);
c.transport.destroy()
}if(g.closeOnDocClick){IN.Event.onOnce(document,"click",i,c.transport)
}else{if(b&&b.closeOnDocClick){IN.Event.remove(document,"click",i,c.transport)
...
switch(this.frameMode){case"embedded":d=this.getEmbeddedRpc(b);
break;
case"hovercard":d=this.getEmbeddedRpc(b);
break;
case"invisible":d=this.getEmbeddedRpc(b);
break;
case"modal":d=this.getEmbeddedRpc(b);
break;
...
a={local:{authorize:function d(p,m,o,q,n){IN.User.setAuthorized(p,m,o);
q()
},logout:function l(n,m){IN.User.setLoggedOut();
n()
},widgetSuccess:function k(n,o,m){e.handleWidgetSuccess(n,o,m)
},widgetError:function h(n,o,m){e.handleWidgetFailure(n,o,m)
...
},resize:function b(r,o,s,p){i.style.width=r+"px";
i.style.height=o+"px";
if(!e.frameOptions.embedded){IN.Objects.Lib.center(i)
}if(!e.frameOptions.embedded&&e.anchorInfo){var q=IN.Objects.Lib.anchor(e.anchorInfo.fixed,i,e.anchorInfo.context);
e.transport.mode({display:"hover",context:q.movable})
}s({width:r,height:o})
},login:function k(p,o){IN.GlobalEvents.refresh.fire()
},close:function m(p,o){e.transport.destroy()
