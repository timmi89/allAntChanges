...
*/
                        ...
                        // $rindow.find('a.rdr_undo_link').click( function() {
                        if ( $rindow.data('mode') == "writeMode" ) {
                            var headerText = "Your reaction to this?";
                        } else {
                            if (kind=="text") {
            ...
            _rindowTypes: {
                ...
                tagMode: {
                    ...
                    make: function(settings){
                        ...
                        if ( settings.mode == "writeMode" ) {
                            // writeMode
                            var headerText = (actionType=="react") ? 'Your reaction to this?':'Save Note';
                        } else {
                            // readMode
            ...
            function _makeSummaryWidget(settings){
                ...
                function writeTag(tag, page) {
                    ...
                    if ( $react.find('a.rdr_tag_'+tag.id).length === 0 ) { // removing tag count check for now:  && $react.find('a.rdr_tag').length < 4
                        ...
                        var $a = $('<a class="rdr_tag rdr_tag_'+tag.id+'">'+tag.body+'</a>').data({
                            tag_id: tag.id,
                            tag_body: tag.body,
                            tag_count: tagCount
                        });
