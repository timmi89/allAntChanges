...
def get_popular_reactions(group, start_date, end_date, mobile):  
    ...
    if mobile:
        clause = BQClause(BQClause(ET_RE, 'AND', DATE_CLAUSE), 'AND', MOBILE)
    else:
        clause = BQClause(BQClause(ET_RE, 'AND', DATE_CLAUSE), 'AND', DESKTOP)
    b.set_clause(clause).set_group_by('group by ev').set_order_by('order by counts desc').build_query().run_query()
...
def rough_score_joined(group, start_date, end_date, mobile):
    ...
    second_join = QueryJoiner(['pvpid','pvcounts','rvcounts', 'rscounts', '((rscounts * 10 + rvcounts) / pvcounts) as score'], 
                              first_join.__str__(), 'agg1', 'left join each', rs_query, 'rs', 'agg1.pvpid = rs.rspid where agg1.pvcounts > 100')
