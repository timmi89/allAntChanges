...
def createInteraction(page, container, content, user, kind, interaction_node, group=None, parent=None, tag_is_default=False):
    ...
    if interaction_node.body == '':
        raise JSONException("Group has blocked this tag. Error code 1.")
    ...
    if checkIfOffensiveTag == True and kind and kind == 'tag':
        ...
        if group and group.blocked_tags:
            for blocked in group.blocked_tags.all():
                if interaction_node.body == blocked.body:
                    raise JSONException("Group has blocked this tag. Error code 2: " + interaction_node.body +" : " + blocked.body)
        ...
        if group and group.word_blacklist:
            ...
            if tagLowerCased in blacklist:
                raise JSONException("Group has blocked this tag. Error code 3: "+tagLowerCased)
            ...
            if tagLowerCasedNoNumbers in blacklist:
                raise JSONException("Group has blocked this tag. Error code 4: "+tagLowerCasedNoNumbers)
            ...
            if tagLowerCased.endswith('er'):
                if tagLowerCased[:-2] in blacklist:
                    raise JSONException("Group has blocked this tag. Error code 5: "+tagLowerCased)
            ...
            for word in interaction_node.body.lower().replace('-', ' ').split(' '):
                ...
                # check the whole reaction (i.e. 'f u c k'), and smash case
                if tagLowerCased in blacklist:
                    raise JSONException("Group has blocked this tag. Error code 7: "+tagLowerCased)
                ...
                # so fuck1 is still "fuck1" but "fuck 1" is now "fuck"
                if tagLowerCasedNoNumbers in blacklist:
                    raise JSONException("Group has blocked this tag. Error code 8: " + tagLowerCasedNoNumbers)

                # let's check for words ending in "er" and see if they match bad words.
                # so check to see if "fucker" --> "fuck" --> blackword match
                if tagLowerCased.endswith('er'):
                    if tagLowerCased[:-2] in blacklist:
                        raise JSONException("Group has blocked this tag. Error code 9: "+tagLowerCased)
    ...
    try:
        ...
        return dict(
            ...
            content_node=content,
            existing=True,
            approved=existing_interaction.approved,
            container=container
        )
    except Interaction.DoesNotExist:
        pass
    ...
    #### requires_approval
    print "approveOnCreate"
    print str(approveOnCreate)
